...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #OryxCoinTEST2\r");
            Send(hSocket, "WHO #OryxCoinTEST2\r");
        } else {
            // randomly join #OryxCoin00-#OryxCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #OryxCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #OryxCoin%02d\r", channel_number).c_str());
        }
